...
namespace AClockworkBerry.Unity.Splines
{
    /// <summary>
    ...
    public class Spline : MonoBehaviour
    {
        ...
        /// <summary>
        /// The number of samples per curve to be used in the approximate arc length reparameterization algorithm.
        /// </summary>
        [HideInInspector]
        private int _nSamples = 10;

        /// <summary>
        /// Gets or sets the number of samples per curve to be used in the approximate arc length reparameterization algorithm.
        /// </summary>
        /// <value>
        /// The n samples.
        /// </value>
        public int nSamples
        {
            ...
            set { _nSamples = value; SetDirty(); }
        }

        /// <summary>
        /// The cache of arc length samples to be used in the approximate arc length reparameterization algorithm.
        /// For each curve _nSamples are saved.
        /// </summary>
        private float[] _sSample;
        ...
        /// <param name="gizmoType">Type of the gizmo.</param>
        [DrawGizmo(GizmoType.NotSelected | GizmoType.Pickable)]
        static private void _RenderSplineGizmo(Spline spline, GizmoType gizmoType)
        {
            Color c = spline.color;
            c.a = 0.5f;

            float size = HandleUtility.GetHandleSize(spline.transform.TransformPoint(spline.GetControlPoint(0)));

            if (spline.showGizmo) Gizmos.DrawIcon(spline.GetPointAtIndex(0), "spline-gizmo");

            if (spline.showNumbers)
            {
                ...
                for (int i = 0; i <= spline.curveCount; i++)
                    Handles.Label(spline.transform.TransformPoint(spline.GetControlPoint(i * 3)) + Vector3.right * size * 0.1f, "" + i, style);
            }
            ...
            for (int i = 0; i < spline.curveCount; i++)
            {
                Vector3 point1 = spline.GetPoint(i, i + 1, 0);
                float step = 1f / 10f;
                for (float t = step; ; t += step)
                {
                    if (t > 1) t = 1;

                    Vector3 point2 = spline.GetPoint(i, i + 1, t);
        ...
        {
            if (_sSample != null && _sSample.Length != 0) return; // Skip if already done.

            _UpdateLengths(); // Make sure we have curve lengths values

            // Allocating arrays. We need nSamples for each curve plus a sample for index 0.
            _sSample = new float[_nSamples * curveCount + 1];
            _tSample = new float[_nSamples * curveCount + 1];
            _tsSlope = new float[_nSamples * curveCount + 1];
            ...
            for (int c = 0; c < curveCount; c++)
            {
                ...
                {
                    int index = c * _nSamples + i;

                    _sSample[index] = _arcLengths[c] + _curveLengths[c] * i / _nSamples;
                    _tSample[index] = GetArcLengthParameter(_sSample[index]);
                    _tsSlope[index] = (_tSample[index] - _tSample[index - 1]) / (_sSample[index] - _sSample[index - 1]);
                }
            }
        }

        /// <summary>
        /// Last curve index used in the last call of GetArcLengthParameterApproximate. 
        /// Next function call will search curve index will be searched starting from this value.
        /// </summary>
        int _lastCurveIndex = 0;
        ...
        public float GetArcLengthParameterApproximate(float s)
        {
            ...
            // Get current curve index, starting from last curve index. 
            // If this function is called for consecutive s, curveIndex will be often the same between calls.
            int curveIndex = 0;
            
            if (s >= _arcLengths[_lastCurveIndex] && s < _arcLengths[_lastCurveIndex + 1]) // current curve index is the same of last curve index
            {
                curveIndex = _lastCurveIndex;
            }
            else if (s >= _arcLengths[_lastCurveIndex + 1]) // current curve index is bigger then last curve index
            {
                for (curveIndex = _lastCurveIndex + 1; curveIndex < curveCount; curveIndex++)
                    if (s < _arcLengths[curveIndex + 1]) break;
            }
            else // current curve index is lesser then last curve index
            {
                for (curveIndex = _lastCurveIndex - 1; curveIndex > 0; curveIndex--)
                    if (s > _arcLengths[curveIndex]) break;
            }

            _lastCurveIndex = curveIndex;

            // Get the sample index n the current curve. Each curve contains nSamples samples.
            int sampleStartIndex = curveIndex * _nSamples;
            int sampleIndex = sampleStartIndex + (int)((s - _sSample[sampleStartIndex]) / _curveLengths[curveIndex] * _nSamples);

            // Return linear interpolation between sampleIndex and sampleIndex + 1 
            return _tSample[sampleIndex] + _tsSlope[sampleIndex + 1] * (s - _sSample[sampleIndex]);
        }
    }
}